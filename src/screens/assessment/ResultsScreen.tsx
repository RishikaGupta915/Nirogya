    ...
    <ScreenWrapper>
      <Animated.View style={heroAnim}>
        <View className="mb-4 flex-row items-center justify-between pt-2">
          ...
          <Text className="text-[19px] text-textPrimary" style={{ fontFamily: FONTS.serif }}>
            Your results
          </Text>
        ...
        >
          ...
          <Text className="mb-2 text-[24px] text-[#d5457a]" style={{ fontFamily: FONTS.serif, fontWeight: '600' }}>
            {diagnosis.diagnosis}
          </Text>
          <Text className="text-[13px] leading-[21px] text-textSecondary" style={{ fontFamily: FONTS.sans }}>
      ...
      <Animated.View style={bodyAnim}>
        <View className="mb-4">
          <View className="mb-2 flex-row justify-between">
            ...
            <Text className="text-[13px] text-textPrimary" style={{ fontFamily: FONTS.sansBold }}>
              {diagnosis.riskScore}%
            </Text>
          </View>
          <View className="h-[6px] overflow-hidden rounded bg-borderSoft">
            <View className="h-full rounded" style={{ width: fillWidth as any, backgroundColor: riskColor }} />
          </View>
        </View>
        ...
        {diagnosis.seeDoctor && (
          <View
            ...
            style={{ backgroundColor: COLORS.amberBg, borderColor: COLORS.amberBorder }}
          >
            <MaterialCommunityIcons name="hospital-box-outline" size={16} color={COLORS.amber} />
            <Text className="text-[12px]" style={{ color: COLORS.amber, fontFamily: FONTS.sans }}>
              See a doctor: <Text style={{ fontFamily: FONTS.sansBold }}>{diagnosis.urgency}</Text>
            </Text>
          </View>
        )}
        ...
        >
          <Text className="mb-3 text-[12px] text-textSecondary" style={{ fontFamily: FONTS.sansBold }}>
            What to do next
          </Text>
          {diagnosis.nextSteps.map((step, i) => (
            <View key={i} className="mb-3 flex-row items-start gap-3">
              ...
              >
                <Text className="text-[10px] font-bold" style={{ color: STEP_COLORS[i % STEP_COLORS.length] }}>
                  {i + 1}
                </Text>
              </View>
              <Text className="flex-1 text-[12px] leading-[19px] text-textSecondary" style={{ fontFamily: FONTS.sans }}>
                {step}
              </Text>
            </View>
          ))}
        ...
        <View className="mb-4 flex-row items-start gap-1">
          <MaterialCommunityIcons name="information-outline" size={12} color={COLORS.textHint} />
          <Text className="flex-1 text-[11px] leading-[17px] text-textHint" style={{ fontFamily: FONTS.sans }}>
            This is not a medical diagnosis. Always consult a qualified doctor before starting treatment.
          </Text>
        </View>

        <GradientButton label="Check another symptom" onPress={() => nav.navigate('Home')} />
        <GhostButton label="View all history" onPress={() => nav.navigate('History')} />
      </Animated.View>
    </ScreenWrapper>
  );
}